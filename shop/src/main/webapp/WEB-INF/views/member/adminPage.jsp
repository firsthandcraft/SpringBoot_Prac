...
<script>
	...
	$("#s1").click(function(){
		var x=0;
		x=this.options[this.options.seletedIndex].value
		sell=x;
		$.post("/damin/getsub",{
			type:2,
			p_id:x//전단계 대분류
			...
			var c= eval("("+data+")");
			$("#s2").empty();//초기화 하려면 비우고
			$("#s3").empty();//초기화 하려면 비우고
			for (i=0; i<c.length;i++){
				$("#s2").append(
					"<option value='"+c[i],id+"'>"
					+c[i].name+"</option>");//추가하고
			}
		});
	});
	$("#s2").click(function(){
...
<body>
	<h3>관리자</h3>
대분류
	<form id="f1"action="${pageContext.request.contextPath}/admin/addCategory" onsubmit="return false" method="post">
...
중분류	
	<form id="f2"action="${pageContext.request.contextPath}/admin/addCategory" onsubmit="return false" method="post">
...
소뷴류
	<form id="f3"action="${pageContext.request.contextPath}/admin/addCategory" onsubmit="return false" method="post">
